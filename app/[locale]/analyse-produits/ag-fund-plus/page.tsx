...
export async function generateMetadata({
  ...
  const t = await getTranslations('analyseAgFundPlus');
  
  return {
    title: t('meta_title'),
    description: t('meta_description'),
    alternates: buildAlternates(locale, '/analyse-produits/ag-fund-plus'),
  }
}
...
export default async function AgFundPlusAnalysisPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <>
      <ArticleJsonLd locale={locale} path="/analyse-produits/ag-fund-plus" namespace="analyseAgFundPlus" />
      ...
      <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6">
        ...
        <section>
          ...
          <div className="mt-6 rounded-xl border border-[var(--warm-tan)]/30 bg-[var(--warm-white)] p-4">
            <p className="text-xs leading-relaxed text-[var(--charcoal)]/60">
              {t('s4_context_note')}
            </p>
          </div>
        ...
        <section className="rounded-3xl bg-[var(--forest-deep)] p-8 text-white md:p-12">
          ...
          <div className="mt-6 space-y-4 opacity-90 text-base leading-relaxed">
            <p>{t('s6_verdict_p1')}</p>
            <p>{t('s6_verdict_p2')}</p>
            <p className="text-sm italic">{t('s6_verdict_p3')}</p>
          </div>
        ...
        <LastUpdated path="/analyse-produits/ag-fund-plus" />
