...
export async function generateMetadata({
  ...
  const t = await getTranslations('analyseBnpComfortEquity');

  return {
    title: t('meta_title'),
    description: t('meta_description'),
    alternates: buildAlternates(locale, '/analyse-produits/bnp-comfort-equity'),
  }
}
...
export default async function BnpComfortEquityPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  ...
  return (
    <>
      <ArticleJsonLd locale={locale} path="/analyse-produits/bnp-comfort-equity" namespace="analyseBnpComfortEquity" />
      ...
      <div className="mx-auto max-w-3xl px-4 py-14 sm:px-6">
        ...
        <section>
          ...
          <Body>{t('s2_comparison')}</Body>
          <WarningBox>{t('s2_warning')}</WarningBox>
          <NeutralCallout>{t('s2_recovery')}</NeutralCallout>
        </section>

        <Divider />

        {/* Section 3: La fiscalité */}
        <section>
          <SectionHeading>{t('s3_title')}</SectionHeading>
          <Body>{t('s3_intro1')}</Body>
          <Body>{t('s3_intro2')}</Body>

          {/* TOB comparison table */}
          <div className="mt-6 overflow-x-auto rounded-2xl border border-[var(--warm-tan)]/50">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-[var(--warm-tan)]/10">
                  <th className="px-4 py-3 font-semibold text-[var(--charcoal)]/40"></th>
                  <th className="px-4 py-3 font-bold text-red-700">{t('s3_col_bnp')}</th>
                  <th className="px-4 py-3 font-bold text-[var(--forest)]">{t('s3_col_etf')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--warm-tan)]/20">
                <tr>
                  <td className="px-4 py-3 text-[var(--charcoal)]/60">{t('s3_row_tob_buy')}</td>
                  <td className="px-4 py-3 text-[var(--charcoal)]">{t('s3_bnp_tob_buy')}</td>
                  <td className="px-4 py-3 text-[var(--charcoal)]">{t('s3_etf_tob_buy')}</td>
                </tr>
                <tr className="bg-[var(--warm-white)]/50">
                  <td className="px-4 py-3 text-[var(--charcoal)]/60">{t('s3_row_tob_sell')}</td>
                  <td className="px-4 py-3 font-semibold text-red-600">{t('s3_bnp_tob_sell')}</td>
                  <td className="px-4 py-3 text-[var(--charcoal)]">{t('s3_etf_tob_sell')}</td>
                </tr>
              </tbody>
            </table>
          </div>

          {/* TOB simulation figures */}
          <div className="mt-6 rounded-xl border border-[var(--warm-tan)]/30 bg-[var(--warm-white)] px-5 py-4 space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--charcoal)]/40">{t('s3_tob_sim_title')}</p>
            <p className="text-sm text-[var(--charcoal)]/75">{t('s3_tob_sim_bnp')}</p>
            <p className="text-sm text-[var(--charcoal)]/75">{t('s3_tob_sim_etf')}</p>
            <p className="text-sm font-semibold text-red-700">{t('s3_tob_sim_diff')}</p>
          </div>

          {/* Reynders exception — sage callout */}
        ...
        <section>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-[var(--warm-tan)]/10">
                  ...
                  <th className="px-3 py-3 font-bold text-red-700">{t('s4_col_bnp')}</th>
              ...
              <tbody className="divide-y divide-[var(--warm-tan)]/20">
                <tr>
                  ...
                  <td className="px-3 py-2 text-red-600">{t('s4_bnp_entry')}</td>
                  <td className="px-3 py-2 text-[var(--charcoal)]">{t('s4_etf_entry')}</td>
                </tr>
                <tr className="bg-[var(--warm-white)]/50">
                  <td className="px-3 py-2 text-[var(--charcoal)]/60">{t('s4_row_net_return')}</td>
                  <td className="px-3 py-2 text-red-600">{t('s4_bnp_return')}</td>
                  <td className="px-3 py-2 text-green-600">{t('s4_etf_return')}</td>
                </tr>
                <tr>
                  <td className="px-3 py-2 text-[var(--charcoal)]/60">{t('s4_row_gross')}</td>
                  <td className="px-3 py-2 text-[var(--charcoal)]">{t('s4_bnp_gross')}</td>
                  <td className="px-3 py-2 text-[var(--charcoal)]">{t('s4_etf_gross')}</td>
                </tr>
                <tr className="bg-[var(--warm-white)]/50">
                  <td className="px-3 py-2 text-[var(--charcoal)]/60">{t('s4_row_tob')}</td>
                  <td className="px-3 py-2 text-red-600">{t('s4_bnp_tob')}</td>
                  <td className="px-3 py-2 text-[var(--charcoal)]">{t('s4_etf_tob')}</td>
                </tr>
                <tr>
                  <td className="px-3 py-2 text-[var(--charcoal)]/60">{t('s4_row_cgt')}</td>
                  <td className="px-3 py-2 text-red-600">{t('s4_bnp_cgt')}</td>
                  <td className="px-3 py-2 text-red-600">{t('s4_etf_cgt')}</td>
                </tr>
                <tr className="border-t-2 border-[var(--warm-tan)]/40">
                  <td className="px-3 py-2 font-bold text-[var(--charcoal)]">{t('s4_row_net_total')}</td>
                  <td className="px-3 py-2 font-bold text-red-600">{t('s4_bnp_net_total')}</td>
                  <td className="px-3 py-2 font-bold text-green-600">{t('s4_etf_net_total')}</td>
                </tr>
                <tr className="bg-[var(--warm-white)]/50">
                  <td className="px-3 py-2 font-bold text-[var(--charcoal)]">{t('s4_row_net_gain')}</td>
                  <td className="px-3 py-2 font-bold text-red-600">{t('s4_bnp_net_gain')}</td>
                  <td className="px-3 py-2 font-bold text-green-600">{t('s4_etf_net_gain')}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          {/* Active funds comparison */}
          <div className="mt-10">
            <p className="mb-3 text-sm font-semibold text-[var(--charcoal)]/60">{t('s4_comparison_title')}</p>
            <div className="overflow-x-auto rounded-2xl border border-[var(--warm-tan)]/50">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="bg-[var(--warm-tan)]/10">
                    <th className="px-3 py-3 font-semibold text-[var(--charcoal)]/40">{t('s4_comp_col_product')}</th>
                    <th className="px-3 py-3 font-semibold text-[var(--charcoal)]/40">{t('s4_comp_col_ter')}</th>
                    <th className="px-3 py-3 font-semibold text-[var(--charcoal)]/40">{t('s4_comp_col_gain')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--warm-tan)]/20">
                  <tr>
                    <td className="px-3 py-2 text-[var(--charcoal)]/80">{t('s4_comp_crelan_name')}</td>
                    <td className="px-3 py-2 text-red-600">{t('s4_comp_crelan_ter')}</td>
                    <td className="px-3 py-2 text-red-600">{t('s4_comp_crelan_gain')}</td>
                  </tr>
                  <tr className="bg-[var(--warm-white)]/50">
                    <td className="px-3 py-2 text-[var(--charcoal)]/80">{t('s4_comp_bnp_name')}</td>
                    <td className="px-3 py-2 text-red-600">{t('s4_comp_bnp_ter')}</td>
                    <td className="px-3 py-2 text-red-600">{t('s4_comp_bnp_gain')}</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 font-semibold text-[var(--forest)]">{t('s4_comp_etf_name')}</td>
                    <td className="px-3 py-2 font-semibold text-green-600">{t('s4_comp_etf_ter')}</td>
                    <td className="px-3 py-2 font-semibold text-green-600">{t('s4_comp_etf_gain')}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p className="mt-2 text-xs text-[var(--charcoal)]/40">{t('s4_comp_note')}</p>
          </div>

          <p className="mt-6 text-sm italic leading-relaxed text-[var(--charcoal)]/45">
            {t('s4_historical_note')}
          </p>

          <p className="mt-4 text-sm italic leading-relaxed text-[var(--charcoal)]/45">
            {t('s4_methodology')}
          </p>
        </section>
        ...
        <section>
          ...
          <NeutralCallout>
            {t('s5_callout')}{' '}
            <Link href="/strategies" className="font-semibold text-[var(--forest)] hover:underline">
              {t('s5_cta_strategies')}
            </Link>
          </NeutralCallout>
        </section>
        ...
        <section className="rounded-3xl bg-[var(--forest-deep)] p-8 text-white md:p-12">
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="rounded-xl bg-white/5 p-4">
              <p className="font-bold text-sm uppercase tracking-wide text-white/50">{t('s6_niche_title')}</p>
              <p className="mt-2 text-sm leading-relaxed">{t('s6_niche_desc')}</p>
            </div>
          </div>
        </section>
        ...
        <div className="mt-12 space-y-4">
          ...
          <Link
            href="/analyse-produits/crelan-invest-opportunities"
        ...
        <LastUpdated path="/analyse-produits/bnp-comfort-equity" />
